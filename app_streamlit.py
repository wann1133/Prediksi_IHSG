...
def build_features(data: pd.DataFrame) -> pd.DataFrame:
    ...
    if close_col is None:
        raise KeyError("Tidak ditemukan kolom 'close' di data.")
...
with st.spinner("📥 Mengambil data IHSG..."):
    raw, source = fetch_data(start, end)
    if raw.empty:
        st.error("❌ Gagal mendapatkan data IHSG dari sumber mana pun.")
        st.stop()
...
# Plotly Chart
st.subheader(f"📊 Grafik Harga IHSG + Prediksi ({horizon_label}) — Sumber Data: {source}")
fig = go.Figure()
